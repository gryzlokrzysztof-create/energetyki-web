...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!drink) return <div className="min-h-screen bg-black flex items-center justify-center font-black text-yellow-400 uppercase italic">Ładowanie...</div>;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-16 mb-16 items-start">
          <div className="rounded-[40px] overflow-hidden border-2 border-zinc-800 bg-zinc-900 shadow-2xl aspect-square cursor-zoom-in group" onClick={() => setFullImage(coverUrl)}>
            <img src={coverUrl} className="w-full h-full object-cover transition-transform group-hover:scale-105" onError={(e: any) => { if (!e.target.src.includes('.jpg')) e.target.src = coverUrl.replace('.JPG', '.jpg'); }} />
          </div>
          ...
          <div className="flex flex-col h-full justify-center">
            ...
            <div className="border-l-4 border-yellow-400 pl-8">
              <p className="text-lg text-zinc-400 italic lowercase first-letter:uppercase leading-relaxed">{drink.description}</p>
            </div>
          </div>
        </div>
          ...
          <div className="md:col-span-2 text-left">
            ...
            <div className="bg-zinc-900/20 p-8 rounded-3xl border border-zinc-800 shadow-inner">
              <p className="text-sm text-zinc-400 leading-relaxed lowercase italic">
                {drink.Ingredients || "Dane w trakcie uzupełniania."}
              </p>
            </div>
          </div>
          <div>
            <h2 className="text-2xl font-black text-white italic mb-6 tracking-tighter text-left text-left">ETYKIETA</h2>
            <div className="rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-900 cursor-zoom-in hover:border-yellow-400 transition-all aspect-[3/4]" onClick={() => setFullImage(ingredientsImgUrl)}>
              <img src={ingredientsImgUrl} className="w-full h-full object-cover opacity-60 hover:opacity-100" onError={(e: any) => { if (!e.target.src.includes('.jpg')) e.target.src = ingredientsImgUrl.replace('.JPG', '.jpg'); }} />
            </div>
          </div>
        </div>
      </div>
